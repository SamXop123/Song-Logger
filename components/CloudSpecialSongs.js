...
export default function CloudSpecialSongs({
  specialSongs,
  onAddSpecialSong,
  onDeleteSpecialSong,
  isLoading,
  ...
  const handleAddMonth = async (e) => {
    ...
    await onAddSpecialSong("month", monthSongInput, monthYearInput, selectedMonth);
    setMonthSongInput("");
  };
  ...
  const handleAddYear = async (e) => {
    ...
    await onAddSpecialSong("year", yearSongInput, selectedYear);
    setYearSongInput("");
  };

  const handleDelete = (id) => {
    if (window.confirm("Delete this special song?")) {
      onDeleteSpecialSong(id);
    }
  };
  ...
  return (
    <div className="bg-white/80 dark:bg-slate-900/40 backdrop-blur-xl p-6 rounded-[2rem] shadow-2xl shadow-indigo-500/5 dark:shadow-[0_8px_30px_rgb(0,0,0,0.4)] border border-white/60 dark:border-slate-700/50 w-full h-full flex flex-col transition-all">
      ...
      <div className="flex flex-col gap-6 overflow-y-auto custom-scrollbar pr-3 flex-grow max-h-[650px] pb-4">
        {isLoading && (
          <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50/70 dark:bg-slate-800/40 px-4 py-6 text-center text-sm text-slate-500 dark:text-slate-400">
            Loading your favorites wall...
          </div>
        )}
        ...
        <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/30">
          ...
          <form onSubmit={handleAddYear} className="flex flex-col gap-2">
            ...
            <button
              type="submit"
              className="w-full bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-400 hover:bg-yellow-200 dark:hover:bg-yellow-900/70 border border-yellow-200 dark:border-yellow-800/50 px-3 py-1.5 rounded-lg text-sm font-medium transition-all"
            >
        ...
        {yearSongs.length > 0 && (
          <div className="flex flex-col gap-2">
            {yearSongs.map((song) => (
              <div
                key={song.id}
                className="group flex justify-between items-center bg-white/60 dark:bg-slate-800/50 p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm transition-all hover:border-yellow-400 dark:hover:border-yellow-600/50"
              >
                ...
                <button
                  ...
                  className="shrink-0 text-slate-400 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100 p-1"
                  title="Remove"
                >
                  <svg
                    className="w-4 h-4"
        ...
        <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/30">
          ...
          <form onSubmit={handleAddMonth} className="flex flex-col gap-2">
            ...
            <button
              type="submit"
              className="w-full bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400 hover:bg-amber-200 dark:hover:bg-amber-900/70 border border-amber-200 dark:border-amber-800/50 px-3 py-1.5 rounded-lg text-sm font-medium transition-all"
            >
        ...
        {monthSongs.length > 0 && (
          <div className="flex flex-col gap-2 pb-4">
            {monthSongs.map((song) => (
              <div
                key={song.id}
                className="group flex justify-between items-center bg-white/60 dark:bg-slate-800/50 p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm transition-all hover:border-amber-300 dark:hover:border-amber-700/50"
              >
                <div className="min-w-0">
        ...
        {!isLoading && specialSongs.length === 0 && (
